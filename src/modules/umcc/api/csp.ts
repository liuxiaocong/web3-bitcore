...
export class ETHStateProvider extends InternalStateProvider implements IChainStateService {
  ...
  constructor(public chain: string = 'UMCC') {
    super(chain);
  ...
  async streamAddressTransactions(params: StreamAddressUtxosParams) {
    ...
    if (!args.tokenAddress) {
      const query = {
        $or: [
          { chain, network, from: {$regex:address,$options:"$i"} },
          { chain, network, to: {$regex:address,$options:"$i"} },
          { chain, network, 'abiType.params.value': {$regex:address,$options:"$i"} }
        ]
      };
